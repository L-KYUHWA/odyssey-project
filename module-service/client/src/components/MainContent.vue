<template>
  <div class="container">
    <keyword-search
      @location-search="onSearchByLocationKeyword">
    </keyword-search>

    <location-list
      :search-result="searchResult"
      @show-detail-modal="showDetailModal"
    ></location-list>

    <location-detail-modal
      ref="detailModal">
    </location-detail-modal>

    <b-pagination-nav
      :link-gen="linkGen"
      :number-of-pages="10"
      use-router>
    </b-pagination-nav>

  </div>
</template>

<script>
  import KeywordSearch from './KeywordSearch'
  import LocationList from "./LocationList";
  import LocationDetailModal from "./LocationDetailModal"

  export default {
    name: "MainContent",
    data() {
      return {
        searchResult: [{
          "address_name": "서울 강남구 삼성동 159",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "418",
          "id": "26338954",
          "phone": "02-6002-1880",
          "place_name": "카카오프렌즈 스타필드 코엑스몰점",
          "place_url": "http://place.map.kakao.com/26338954",
          "road_address_name": "서울 강남구 영동대로 513",
          "x": "127.059028716089",
          "y": "37.5120756277877"
        }, {
          "address_name": "서울 서초구 서초동 1305-7",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "3596",
          "id": "653245473",
          "phone": "02-6494-1100",
          "place_name": "카카오프렌즈 강남플래그십스토어",
          "place_url": "http://place.map.kakao.com/653245473",
          "road_address_name": "서울 서초구 강남대로 429",
          "x": "127.025811882515",
          "y": "37.5008934694709"
        }, {
          "address_name": "서울 송파구 신천동 29",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "3679",
          "id": "27375630",
          "phone": "02-3213-4514",
          "place_name": "카카오프렌즈 롯데월드몰 잠실점",
          "place_url": "http://place.map.kakao.com/27375630",
          "road_address_name": "서울 송파구 올림픽로 300",
          "x": "127.104446099818",
          "y": "37.5137525026039"
        }, {
          "address_name": "서울 송파구 신천동 29",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "3511",
          "id": "763114469",
          "phone": "02-3213-6743",
          "place_name": "카카오프렌즈 롯데월드타워 면세점",
          "place_url": "http://place.map.kakao.com/763114469",
          "road_address_name": "서울 송파구 올림픽로 300",
          "x": "127.102493347363",
          "y": "37.5125829083359"
        }, {
          "address_name": "서울 서초구 반포동 19-3",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "5267",
          "id": "27255007",
          "phone": "02-3479-1328",
          "place_name": "카카오프렌즈 신세계백화점 강남점",
          "place_url": "http://place.map.kakao.com/27255007",
          "road_address_name": "서울 서초구 신반포로 176",
          "x": "127.004351994592",
          "y": "37.5052714927176"
        }, {
          "address_name": "서울 중구 을지로7가 143",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "7507",
          "id": "329600907",
          "phone": "02-2231-3552",
          "place_name": "카카오프렌즈 DDP동대문점",
          "place_url": "http://place.map.kakao.com/329600907",
          "road_address_name": "서울 중구 을지로 281",
          "x": "127.009865740324",
          "y": "37.5672059407741"
        }, {
          "address_name": "서울 중구 충무로1가 52-5",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "8887",
          "id": "1777071768",
          "phone": "02-6370-4278",
          "place_name": "카카오프렌즈 신세계면세 명동점",
          "place_url": "http://place.map.kakao.com/1777071768",
          "road_address_name": "서울 중구 소공로 63",
          "x": "126.981089600579",
          "y": "37.5609663400347"
        }, {
          "address_name": "서울 중구 남대문로2가 123",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "9014",
          "id": "1044004404",
          "phone": "02-2118-5150",
          "place_name": "카카오프렌즈 롯데영플라자명동점",
          "place_url": "http://place.map.kakao.com/1044004404",
          "road_address_name": "서울 중구 남대문로 67",
          "x": "126.981682046205",
          "y": "37.5635252637636"
        }, {
          "address_name": "서울 용산구 한강로3가 40-999",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "8843",
          "id": "26945558",
          "phone": "02-490-3927",
          "place_name": "카카오프렌즈 용산신라아이파크면세점",
          "place_url": "http://place.map.kakao.com/26945558",
          "road_address_name": "서울 용산구 한강대로23길 55",
          "x": "126.964665861534",
          "y": "37.5296762726822"
        }, {
          "address_name": "서울 마포구 동교동 165-5",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "13132",
          "id": "1786432255",
          "phone": "02-6010-0104",
          "place_name": "카카오프렌즈 홍대플래그십스토어",
          "place_url": "http://place.map.kakao.com/1786432255",
          "road_address_name": "서울 마포구 양화로 162",
          "x": "126.923919460392",
          "y": "37.5563194254356"
        }, {
          "address_name": "서울 마포구 동교동 165-5",
          "category_group_code": "CT1",
          "category_group_name": "문화시설",
          "category_name": "문화,예술 \u003e 문화시설 \u003e 박물관",
          "distance": "13149",
          "id": "316668520",
          "phone": "1577-6263",
          "place_name": "카카오프렌즈 컨셉 뮤지엄",
          "place_url": "http://place.map.kakao.com/316668520",
          "road_address_name": "서울 마포구 양화로 162",
          "x": "126.923810614655",
          "y": "37.5565139706643"
        }, {
          "address_name": "서울 영등포구 여의도동 23",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "12199",
          "id": "26945557",
          "phone": "02-6137-5550",
          "place_name": "카카오프렌즈 CGV여의도점",
          "place_url": "http://place.map.kakao.com/26945557",
          "road_address_name": "서울 영등포구 국제금융로 10",
          "x": "126.925534613807",
          "y": "37.52544860226"
        }, {
          "address_name": "서울 영등포구 영등포동4가 442",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "14128",
          "id": "1411970905",
          "phone": "02-2638-2750",
          "place_name": "카카오프렌즈 타임스퀘어점",
          "place_url": "http://place.map.kakao.com/1411970905",
          "road_address_name": "서울 영등포구 영중로 15",
          "x": "126.90306008669398",
          "y": "37.51710703022398"
        }, {
          "address_name": "서울 서대문구 창천동 30-33",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "12145",
          "id": "26029863",
          "phone": "02-3145-1722",
          "place_name": "카카오프렌즈 현대백화점 신촌점",
          "place_url": "http://place.map.kakao.com/26029863",
          "road_address_name": "서울 서대문구 신촌로 83",
          "x": "126.935798501194",
          "y": "37.5560525790381"
        }, {
          "address_name": "경기 성남시 분당구 백현동 541",
          "category_group_code": "",
          "category_group_name": "",
          "category_name": "가정,생활 \u003e 문구,사무용품 \u003e 디자인문구 \u003e 카카오프렌즈",
          "distance": "14245",
          "id": "26944943",
          "phone": "031-5170-2451",
          "place_name": "카카오프렌즈 현대백화점판교점",
          "place_url": "http://place.map.kakao.com/26944943",
          "road_address_name": "경기 성남시 분당구 판교역로146번길 20",
          "x": "127.112385409653",
          "y": "37.392202184912"
        }],
        totalItemCount: 0,
        pageSize: 0,
        currentPage: 0,
      }
    },
    components: {
      LocationList,
      KeywordSearch,
      LocationDetailModal
    },
    methods: {
      onSearchByLocationKeyword(keyword) {
        console.log(keyword);

        fetch("/api/search?keyword=" + keyword + "&page=" + this.currentPage + "&size=" + this.pageSize, {
          method: 'GET'
        }).then((response) => {
          return response.json();
        }).then((data) => {
          if ('FAIL' === data.status) {
            alert("Fail to get search result");
          }

          this.searchResult = data.result.documents;
          this.totalItemCount = data.result.meta.total_count;
          this.pageSize = data.result.meta.pageable_count;

        });
      },
      showDetailModal(selectedItem) {
        this.$refs.detailModal.showModal(selectedItem);
      }
    }
  }
</script>

<style scoped>

</style>
